...
async def speedtest_command(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        # Get the share URL (already includes .png)
...
└ <b>Note:</b> <code>Shows bot server network, not your location</code>"""
        ...
        # Download and send the speedtest image
        try:
            import urllib.request
            
            # Download the image
            image_path = f"speedtest_{user_id}.png"
            urllib.request.urlretrieve(share_url, image_path)
            
            # Delete the status message
            await status_msg.delete()
            
            # Send the image with results as caption
            ...
            logger.error(f"Failed to download/send speedtest image: {img_error}")
            # Fallback to text with link if image fails
            await status_msg.edit_text(
                text=speed_text + f"\n\n<b>🖼️ Result Image:</b> <a href='{share_url}'>View Graph</a>",
                parse_mode=ParseMode.HTML,
