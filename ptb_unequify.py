...
async def receive_target(update: Update, context: CallbackContext) -> int:
    ...
    if update.message.text:
        # Enhanced regex to support all Telegram link formats including private channels
        regex = re.compile(
            r"(?:https?://)?(?:t\.me|telegram\.me|telegram\.dog)/"
            r"(?:c/)?(-?\d+|[a-zA-Z_0-9]+)/(\d+)(?:\?single)?",
            re.IGNORECASE
        )
        match = regex.search(update.message.text.strip())
        ...
        if match:
            ...
            if chat_id.lstrip('-').isdigit():
                # Numeric ID (could be from c/ link or direct ID)
                chat_id_num = int(chat_id)
                # If it's a positive number from c/ link, prefix with -100
                if chat_id_num > 0:
                    chat_id = int(f"-100{chat_id}")
                else:
                    chat_id = chat_id_num
            # else: username string, keep as is
        else:
            await update.message.reply_text(
                ...
                "• https://t.me/c/1234567/123 (private channel)\n"
                "• t.me/channelname/123\n"
                "• Forwarded message from the channel\n\n"
                "<b>💡 Make sure to copy the complete message link from Telegram.</b>",
                parse_mode=ParseMode.HTML
            )
            return WAITING_FOR_TARGET

    # Check if it's a forwarded message (works for public channels)
        ...
        if hasattr(update.message.forward_origin, 'chat'):
            ...
            forward_chat = update.message.forward_origin.chat
            if forward_chat.type in ['channel', 'supergroup']:
                last_msg_id = update.message.forward_origin.message_id
                chat_id = forward_chat.username or forward_chat.id
        elif hasattr(update.message.forward_origin, 'sender_chat'):
            # Forward from anonymous admin or channel
            forward_chat = update.message.forward_origin.sender_chat
            if forward_chat.type in ['channel', 'supergroup']:
                last_msg_id = update.message.forward_origin.message_id if hasattr(update.message.forward_origin, 'message_id') else None
                chat_id = forward_chat.username or forward_chat.id

    # Fallback: Check old-style forward attributes for compatibility
    ...
    # If still no chat_id found, show error
    if not chat_id or not last_msg_id:
        await update.message.reply_text(
            ...
            "<b>💡 For private channels, use the message link (tap message → Copy Link).</b>",
            parse_mode=ParseMode.HTML
        )
        return WAITING_FOR_TARGET
