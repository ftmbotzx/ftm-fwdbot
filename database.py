...
class Database:
    ...
    def new_user(self, id, name):
        ...
        return dict(
            ...
            process_tracking=dict(
                last_ongoing_process = None,  # Dict with process details
                last_forward_msg_id = None,   # Last forwarded message ID
                last_fetched_msg_id = None,   # Last fetched message ID
                last_message_link = None,     # Link to last processed message
                target_channel_link = None,   # Target channel link
                from_chat = None,             # Source chat ID
                to_chat = None,               # Target chat ID
                original_skip = 0,            # Original skip value when process started
                total_messages = 0,           # Total messages in process
                processed_count = 0,          # Number of messages processed
    ...
    async def get_configs(self, id):
        default = {
            ...
            'ftm_prefix': None,  # FTM Watermark prefix
            'ftm_suffix': None,  # FTM Watermark suffix
            'protect': None,
            'filters': {
                'text': True,
                'photo': True, 
    ...
    # Process tracking for restart recovery
    async def update_process_tracking(self, user_id, tracking_data):
        """Update user's process tracking data - merges with existing data"""
        tracking_data['last_updated'] = datetime.utcnow()
        
        # Ensure user exists in database first
        user = await self.get_user(user_id)
        if not user:
            logger.error(f"Cannot update process tracking: user {user_id} not found in database")
            # Try to create user if they don't exist (this shouldn't happen normally)
            logger.warning(f"Attempting to create user {user_id} on-the-fly for process tracking")
            try:
                await self.add_user(user_id, f"User_{user_id}")
                logger.info(f"Created user {user_id} successfully")
            except Exception as e:
                logger.error(f"Failed to create user {user_id}: {e}")
                return None
        
        # Get current tracking data to merge with new data
        current = await self.get_process_tracking(user_id)
        if current:
            # Merge current data with new data (new data overwrites existing fields)
            merged_data = {**current, **tracking_data}
        else:
            # No existing data, use new data as-is
            merged_data = tracking_data
        
        result = await self.col.update_one(
            {'id': int(user_id)},
            {'$set': {'process_tracking': merged_data}},
            upsert=False
        )
        
        # Log if update failed
        if result.matched_count == 0:
            logger.error(f"Process tracking update failed for user {user_id} - no document matched")
        elif result.modified_count == 0:
            logger.warning(f"Process tracking update for user {user_id} matched but made no changes")
        else:
            logger.debug(f"Process tracking updated successfully for user {user_id}")
        
        return result

    async def get_process_tracking(self, user_id):
        """Get user's process tracking data"""
        user = await self.get_user(user_id)
        if user:
            return user.get('process_tracking', {
                'last_ongoing_process': None,
                'last_forward_msg_id': None,
                'last_fetched_msg_id': None,
    ...
    async def get_users_with_ongoing_processes(self):
        """Get all users who have truly ongoing processes (not completed/cancelled)"""
        users = await self.col.find({
            'process_tracking.last_ongoing_process': {'$ne': None},
            'process_tracking.process_completed': {'$ne': True},
            'process_tracking.process_cancelled': {'$ne': True}
        }).to_list(length=1000)
        return users

    async def clear_process_tracking(self, user_id):
        """Clear user's process tracking data"""
        default_tracking = {
            'last_ongoing_process': None,
            'last_forward_msg_id': None,
            'last_fetched_msg_id': None,
        ...
        return await self.col.update_one(
            {'id': int(user_id)},
            {'$set': {'process_tracking': default_tracking}}
        )
    ...
    async def _check_and_complete_referral(self, user_id):
        ...
        # Find any incomplete referral for this user
        referral = await self.referral_col.find_one({
            'referred_user_id': int(user_id),
            'completed': False
        })
        
        # If no incomplete referral found, return False
        if not referral:
            logger.info(f"No incomplete referral found for user {user_id}")
            return False
        
        # Complete the referral regardless of subscription status
        # The user clicked the referral link and verified, that's what matters
        ...
        await self.referral_col.update_one(
            ...
            {'$set': {
                ...
                'channels_joined': True,  # Mark as joined since they verified
                'bot_started': True  # Mark as started since they're using the bot
            }}
        )
