...
def setup_callback_handlers(application):
    ...
    # NOTE: Plan and premium callbacks (my_plan, premium_plans, buy_plan, copy_upi) 
    # are handled by Pyrogram in plugins/premium.py - Do NOT add PTB handlers
    ...
    # NOTE: contact_admin callback is handled by Pyrogram in plugins/public.py
    # NOTE: check_subscription callback is handled by Pyrogram in plugins/fsub.py
    # NOTE: All reset callbacks are handled by Pyrogram in plugins/reset.py
    # Do NOT add PTB handlers for these to avoid conflicts
