...
async def pub_(bot, message, resume_msg=None):
    ...
    # Use resume message if provided (from auto-resume), otherwise use callback message
    if resume_msg:
        m = await msg_edit(resume_msg, "<code>verifying your data's, please wait.</code>")
    else:
    ...
    if Config.is_sudo_user(user):
        ...
        if not can_process:
            if reason == "monthly_limit_reached":
                ...
                limit_msg = f"""<b>🚫 Monthly Limit Reached!</b>
...
<b>📋 Available Plans:</b>
• Plus 15 days: ₹{Config.PLAN_PRICING['plus']['15_days']}
• Plus 30 days: ₹{Config.PLAN_PRICING['plus']['30_days']}
• Pro 15 days: ₹{Config.PLAN_PRICING['pro']['15_days']}
• Pro 30 days: ₹{Config.PLAN_PRICING['pro']['30_days']}

<b>Payment UPI:</b> <code>{Config.UPI_ID}</code>
...
<b>Next reset:</b> 1st of next month"""
    ...
    try:
      ...
      # Make sure m exists before trying to edit it
      if m:
          return await m.edit(error_msg)
      else:
          # If m is None, send a new message instead
          await bot.send_message(user, error_msg)
          return
    ...
    if locked:
        try:
          ...
          # Initialize process tracking for restart recovery
          from plugins.ftm_utils import create_source_link, create_target_link
          await db.update_process_tracking(user, {
              'last_ongoing_process': 'forwarding',
              'last_forward_msg_id': None,
              'last_fetched_msg_id': None,  # Will be updated as we process messages
              'last_message_link': None,
              'target_channel_link': create_target_link(sts.get('TO'), 1),
              'from_chat': sts.get('FROM'),
              'to_chat': sts.get('TO'),
              'original_skip': sts.get('skip'),  # Store original skip value
              'total_messages': sts.get('total'),
              'processed_count': 0
          })
          ...
          async for message in client.iter_messages(
            ...
            offset=int(sts.get('skip')) if sts.get('skip') else 0
                ...
                # Check if message supports captions (stickers, polls, contacts, locations don't support captions)
                supports_caption = bool(message.photo or message.video or message.document or 
                ...
                # Check if watermarks are configured
                watermark_prefix = configs.get('ftm_prefix')
                watermark_suffix = configs.get('ftm_suffix')
                has_watermark = bool(watermark_prefix or watermark_suffix)

                # Force ALL messages to be copied individually when FTM mode OR watermarks are enabled
                # Only use batch forwarding for text-only messages when forward_tag is enabled AND FTM is disabled AND no watermarks
                if forward_tag and not has_media and not ftm_mode and not has_watermark:
                   ...
                   if ( notcompleted >= 100 
                      ...
                      await forward(client, MSG, m, sts, protect, False, _bot['is_bot'])
                   ...
                   try:
                       ...
                       if ftm_mode:
                           # FTM Delta mode - First apply custom caption (for media only), then add FTM header on top
                           source_link = create_source_link(sts.get('FROM'), message.id)

                           # For media messages with custom caption support (not stickers, polls, contacts, locations)
                           if has_media and supports_caption:
                               # Extract file information for caption variables
                               filename = ""
                               filesize = ""
                               media_type = None
                               ...
                               if message.video:
                                   ...
                                   filesize = get_size(message.video.file_size) if hasattr(message.video, 'file_size') else ""
                               elif message.audio:
                                   media_type = "audio"
                                   filename = getattr(message.audio, 'file_name', None) or 'audio.mp3'
                                   filesize = get_size(message.audio.file_size) if hasattr(message.audio, 'file_size') else ""
                               elif message.document:
                                   media_type = "document"
                                   filename = getattr(message.document, 'file_name', None) or 'document'
                                   filesize = get_size(message.document.file_size) if hasattr(message.document, 'file_size') else ""

                               # Prepare message dict for custom caption processing
                               msg_dict = {
                               ...
                               # Apply custom caption first (only for video/audio/documents)
                               processed_caption = custom_caption(msg_dict, caption)

                               # Apply watermark BEFORE FTM Delta footer so suffix appears above FTM text
                               from plugins.ftm_utils import apply_watermark_before_ftm
                               configs = await get_configs(user)
                               watermark_prefix = configs.get('ftm_prefix')
                               watermark_suffix = configs.get('ftm_suffix')
                               if watermark_prefix or watermark_suffix:
                                   processed_caption = apply_watermark_before_ftm(processed_caption, watermark_prefix, watermark_suffix)

                               # Add FTM Delta formatted footer (watermarks already applied)
                               content_with_ftm = add_ftm_caption(processed_caption, source_link)

                               # Send media with FTM format
                               await client.copy_message(
                                   chat_id=sts.get('TO'),
                                   from_chat_id=sts.get('FROM'),
                                   message_id=message.id,
                                   caption=content_with_ftm,
                                   protect_content=protect
                               )
                           elif has_media and not supports_caption:
                               # Non-captionable media (stickers, polls, etc.) - just copy without watermark
                               await client.copy_message(
                                   chat_id=sts.get('TO'),
                                   from_chat_id=sts.get('FROM'),
                                   message_id=message.id,
                               ...
                               # Text-only message - NO custom caption, just add FTM header to original text
                               original_text = message.text if message.text else ""
                               
                               # Apply watermark BEFORE FTM Delta footer
                               from plugins.ftm_utils import apply_watermark_before_ftm
                               configs = await get_configs(user)
                               watermark_prefix = configs.get('ftm_prefix')
                               watermark_suffix = configs.get('ftm_suffix')
                               if watermark_prefix or watermark_suffix:
                                   original_text = apply_watermark_before_ftm(original_text, watermark_prefix, watermark_suffix)
                               
                               # Add FTM Delta footer (watermarks already applied)
                               content_with_ftm = add_ftm_caption(original_text, source_link)

                               # Use custom button if configured, otherwise preserve original inline keyboard
                               reply_markup = button if button else (message.reply_markup if hasattr(message, 'reply_markup') else None)

                               await client.send_message(
                                   chat_id=sts.get('TO'),
                                   text=content_with_ftm,
                                   reply_markup=reply_markup,
                                   protect_content=protect
                               )
                       elif (caption is not None or button) and has_media:
                           # Has custom caption or button AND media (not text-only)
                           # Extract file information for caption variables
                           filename = ""
                           filesize = ""
                           media_type = None
                           ...
                           details = {
                               "msg_id": message.id, 
                               "media": media(message), 
                               "caption": caption,  # User's custom caption template
                               "original_caption": original_caption,  # Message's original caption
                               "filename": filename,
                               "filesize": filesize,
                               "media_type": media_type,
                               'button': button, 
                       ...
                       elif button and not has_media:
                           # Text message with button only (no custom caption applied to text)
                           ...
                           watermark_suffix = configs.get('ftm_suffix')
                           if watermark_prefix or watermark_suffix:
                               from plugins.ftm_utils import apply_watermark
                               text_content = apply_watermark(text_content, watermark_prefix, watermark_suffix)
                           ...
                           # Simple copy - apply watermark if configured
                           configs = await get_configs(user)
                           watermark_prefix = configs.get('ftm_prefix')
                           watermark_suffix = configs.get('ftm_suffix')
                           ...
                           if watermark_prefix or watermark_suffix:
                               from plugins.ftm_utils import apply_watermark
                               # Get original content (text or caption)
                               original_content = message.text if message.text else (message.caption if message.caption else "")
                               watermarked_content = apply_watermark(original_content, watermark_prefix, watermark_suffix)
                               
                               # Send with watermarked caption/text
                               if has_media and supports_caption:
                                   # For captionable media, use copy_message with watermarked caption
                                   await client.copy_message(
                                       chat_id=sts.get('TO'),
                                       from_chat_id=sts.get('FROM'),
                                       message_id=message.id,
                                       caption=watermarked_content,
                                       protect_content=protect
                                   )
                               elif has_media and not supports_caption:
                                   # Non-captionable media (stickers, polls, etc.) - copy without caption
                                   await client.copy_message(
                                       chat_id=sts.get('TO'),
                                       from_chat_id=sts.get('FROM'),
                                       message_id=message.id,
                                   ...
                                   # For text-only, use send_message
                                   await client.send_message(
                                       chat_id=sts.get('TO'),
                               ...
                               # No watermark configured, simple copy
                               await client.copy_message(
                                   chat_id=sts.get('TO'),
                                   from_chat_id=sts.get('FROM'),
                                   message_id=message.id,
                       ...
                       # Update process tracking on every message for restart recovery
                       source_link = create_source_link(sts.get('FROM'), message.id)
                       await db.update_process_tracking(user, {
                           'last_ongoing_process': 'forwarding',
                           'last_forward_msg_id': message.id,
                           'last_fetched_msg_id': message.id,  # Track actual message ID, not fetch count
                           'last_message_link': source_link,
                           'target_channel_link': create_target_link(sts.get('TO'), 1),
                           'from_chat': sts.get('FROM'),
                           'to_chat': sts.get('TO'),
                           'total_messages': sts.get('total'),
                           'processed_count': sts.get('total_files')
                       })
            ...
            # Safely remove from list only if present
            if sts.TO in temp.IS_FRWD_CHAT:
                temp.IS_FRWD_CHAT.remove(sts.TO)

            # Keep process tracking data for historical reference and potential resume
        ...
        # Mark process as completed but don't clear - keep for historical reference
        await db.update_process_tracking(user, {
            'last_ongoing_process': None,  # Mark as no longer ongoing
...
async def copy(bot, msg, m, sts):
   user = m.from_user.id if hasattr(m, 'from_user') else None
   try:
     if 'button' in msg and msg['button']:
        ...
        if msg.get('ftm_mode', False):
           ...
           # First apply custom caption
           user_custom_caption = msg.get('caption')  # User's custom caption template
           final_caption = custom_caption(msg, user_custom_caption)

           # Apply watermark BEFORE FTM footer for text messages with button
           if user:
               from plugins.ftm_utils import apply_watermark_before_ftm
               configs = await get_configs(user)
               watermark_prefix = configs.get('ftm_prefix')
               watermark_suffix = configs.get('ftm_suffix')
               if watermark_prefix or watermark_suffix:
                   final_caption = apply_watermark_before_ftm(final_caption, watermark_prefix, watermark_suffix)

           # Add FTM footer (watermarks already applied)
           caption_with_ftm = add_ftm_caption(final_caption, source_link)
           ...
           if sent_msg and not msg.get('is_bot', True):
              ...
              updated_caption = add_ftm_caption(final_caption, source_link, target_link)
              try:
                 await sent_msg.edit_text(updated_caption, reply_markup=msg['button'])
              except Exception as edit_e:
                 print(f"Failed to edit message with target link: {edit_e}")
        else:
           # Apply custom caption and send with button (non-FTM mode)
           user_custom_caption = msg.get('caption')  # User's custom caption template
           final_caption = custom_caption(msg, user_custom_caption)
           
           # Apply watermark even when FTM mode is disabled
           if user:
               from plugins.ftm_utils import apply_watermark
               configs = await get_configs(user)
               watermark_prefix = configs.get('ftm_prefix')
               watermark_suffix = configs.get('ftm_suffix')
               if watermark_prefix or watermark_suffix:
                   final_caption = apply_watermark(final_caption, watermark_prefix, watermark_suffix)
           
           await bot.send_message(sts.get('TO'), final_caption, reply_markup=msg['button'], protect_content=msg['protect'])
     else:
        media_file_id = msg['media']
        if media_file_id:
           # Get user's custom caption template
           user_custom_caption = msg.get('caption')  # User's custom caption template from configs

           # Apply custom caption processing (handles variable replacement)
           caption = custom_caption(msg, user_custom_caption)

           # Get watermark settings from message details or fetch from configs
           from plugins.ftm_utils import apply_watermark, apply_watermark_before_ftm
           watermark_prefix = msg.get('watermark_prefix')
           watermark_suffix = msg.get('watermark_suffix')
           
           # If not in message details, fetch from configs
           if (watermark_prefix is None and watermark_suffix is None) and user:
               configs = await get_configs(user)
               watermark_prefix = configs.get('ftm_prefix')
               watermark_suffix = configs.get('ftm_suffix')
           ...
           if msg.get('ftm_mode', False):
              ...
              # Apply watermark BEFORE FTM footer (suffix appears above FTM text)
              if watermark_prefix or watermark_suffix:
                  caption = apply_watermark_before_ftm(caption, watermark_prefix, watermark_suffix)
              ...
              if sent_msg and not msg.get('is_bot', True):
                 target_link = create_target_link(sts.get('TO'), sent_msg.id)
                 updated_caption = add_ftm_caption(caption, source_link, target_link)
                 try:
                    await sent_msg.edit_caption(updated_caption)
                 except Exception as edit_e:
                    print(f"Failed to edit caption with target link: {edit_e}")
           else:
              # Normal copy without FTM - apply watermarks if configured
              if watermark_prefix or watermark_suffix:
                  caption = apply_watermark(caption, watermark_prefix, watermark_suffix)
              
              # Normal copy - compatible with both bots and userbots
              try:
                  ...
                  try:
                      ...
                      try:
                          ...
                          print(f"Forward messages also failed: {forward_error2}")
           ...
           if msg.get('ftm_mode', False):
              ...
              # Apply watermark BEFORE FTM footer (suffix appears above FTM text)
              if watermark_prefix or watermark_suffix:
                  text_content = apply_watermark_before_ftm(text_content, watermark_prefix, watermark_suffix)
              ...
              if sent_msg and not msg.get('is_bot', True):
                 target_link = create_target_link(sts.get('TO'), sent_msg.id)
                 # Apply watermark before FTM footer for the update too
                 updated_content = text_content
                 if watermark_prefix or watermark_suffix:
                     updated_content = apply_watermark_before_ftm(text_content, watermark_prefix, watermark_suffix)
                 updated_text = add_ftm_caption(updated_content, source_link, target_link)
                 try:
                    await sent_msg.edit_text(updated_text)
                 except Exception as edit_e:
                    print(f"Failed to edit text with target link: {edit_e}")
           else:
              # Apply watermark for non-FTM mode
              if watermark_prefix or watermark_suffix:
                  text_content = apply_watermark(text_content, watermark_prefix, watermark_suffix)
...
async def forward(bot, msg, m, sts, protect, ftm_mode=False, is_bot=True):
   try:
     if ftm_mode:
        # For FTM mode, copy messages individually to add source links in captions
        for msg_id in msg:
           try:
              ...
              if original_msg and not original_msg.empty and not original_msg.service:
                 ...
                 # Add FTM info to caption/text with source link only (no button)
                 caption = original_msg.caption if original_msg.caption else (original_msg.text if original_msg.text else "")
                 caption_with_source = add_ftm_caption(caption, source_link)
                 ...
                 sent_msg = await bot.copy_message(
                    ...
                    from_chat_id=sts.get('FROM'),
                    message_id=msg_id,
                    caption=caption_with_source if original_msg.media else None,
                    protect_content=protect
                 )

                 # Update with target link for userbots
                 if sent_msg:
                    target_link = create_target_link(sts.get('TO'), sent_msg.id)
                    updated_caption = add_ftm_caption(caption, source_link, target_link)
                    try:
                       if original_msg.media:
                          await sent_msg.edit_caption(caption=updated_caption)
                       else:
                          await sent_msg.edit_text(updated_caption)
                    except Exception as edit_e:
                       print(f"Failed to edit with target link: {edit_e}")
...
async def edit(msg, title, status, sts, force=False):
   ...
   # Progress = current position / total messages (fetched / total)
   percentage = "{:.0f}".format(float(i.fetched)*100/float(i.total) if i.total > 0 else 0)

   now = time.time()
   diff = int(now - i.start)

   # Use actual forwarded count (total_files) for speed calculation
   actual_forwarded = i.total_files  # This is the actual count of forwarded messages
   remaining_to_process = i.total - i.fetched  # Remaining messages based on position

   # Calculate speed based on actual forwarded messages over elapsed time
   speed = sts.divide(actual_forwarded, diff) if diff > 0 else 0
   elapsed_time = round(diff) * 1000
   time_to_completion = round(sts.divide(remaining_to_process, speed)) * 1000 if speed > 0 else 0
   estimated_total_time = elapsed_time + time_to_completion  

   # Calculate speed in messages per minute using actual forwarded count
   speed_per_min = int(speed * 60) if speed > 0 else 0
   speed_text = f"{speed_per_min} msgs" if speed_per_min > 0 else "0 msgs"

   # Create progress bar with filled circles based on percentage (0-100% = 0-11 circles)
   filled_count = math.floor(int(percentage) / 10)  # 0-10 filled circles
   if int(percentage) == 100:
       filled_count = 11  # All 11 circles for 100%
   progress = ''.join(["◉" for j in range(filled_count)])
   ...
   # TEXT template: total, fetched, successfully_fwd, duplicate, deleted/filtered, skipped, status, progress%, speed, eta, progress_bar
   text = TEXT.format(i.total, i.fetched, i.total_files, i.duplicate, filtered_deleted, i.skip, status, percentage, speed_text, estimated_total_time, progress)
...
async def is_cancelled(client, user, msg, sts):
   if temp.CANCEL.get(user)==True:
      ...
      # Mark as cancelled but keep data for historical reference
      await db.update_process_tracking(user, {
          'last_ongoing_process': None,  # Mark as no longer ongoing
          'process_cancelled': True,
          'cancellation_time': dt.utcnow()
      })
...
def custom_caption(msg, user_custom_caption):
    """
    Apply custom caption with variable replacement.
    Custom caption is only applied for video, audio, and documents.
    Variables: {filename}, {filesize}/{size}, {caption}
    """
    media_type = msg.get('media_type')

    # Only apply custom caption for video, audio, and documents
    if media_type not in ['video', 'audio', 'document']:
        # For other media types, return original caption
        return msg.get('original_caption', '')

    # Get values for replacement (ensure they're strings, not None)
    filename = msg.get('filename') or ''
    filesize = msg.get('filesize') or ''
    original_caption = msg.get('original_caption') or ''

    # If no user custom caption is set, return original caption
    if not user_custom_caption:
        return original_caption

    # Replace variables in custom caption
    new_caption = user_custom_caption
    new_caption = new_caption.replace('{filename}', filename)
    new_caption = new_caption.replace('{filesize}', filesize)
    new_caption = new_caption.replace('{size}', filesize)  # Support both {size} and {filesize}
    new_caption = new_caption.replace('{caption}', original_caption)
