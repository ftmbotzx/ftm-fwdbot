        ...
        try:
          ...
          async for message in client.iter_messages(
            ...
            offset=int(sts.get('skip')) if sts.get('skip') else 0
                ...
                # Force ALL messages to be copied individually when FTM mode is enabled
                # Only use batch forwarding for text-only messages when forward_tag is enabled AND FTM is disabled
                if forward_tag and not has_media and not ftm_mode:
                   ...
                   try:
                       ...
                       if ftm_mode:
                           # FTM Delta mode - add formatted source link to ALL messages
                           ...
                           # Add FTM Delta formatted header
                           ftm_header = f"🔥 <b>FTM Delta Mode</b> 🔥\n📤 <b>Source:</b> <a href='{source_link}'>Original Message</a>\n\n"
                           content_with_ftm = ftm_header + original_content if original_content else ftm_header.strip()

                           # Send with FTM format
                           if message.media:
                               ...
                               # Text-only message - send as text
   ...
   try:
     if 'button' in msg and msg['button']:
        ...
        if msg.get('ftm_mode', False):
           ...
           # Add FTM info to caption only (no button)
        ...
        if media_file_id:
           ...
           if msg.get('ftm_mode', False):
              ...
              # Add FTM info to text content only (no button)
