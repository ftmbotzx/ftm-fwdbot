...
async def run(bot, message):
    ...
    force_sub_msg = await send_force_subscribe_message(message, bot)
    if force_sub_msg:
        return
    
    _bot = await db.get_bot(user_id)
    if not _bot:
      return await message.reply(Translation.NO_BOT_ADDED_MSG)
    channels = await db.get_user_channels(user_id)
    if not channels:
       return await message.reply_text(Translation.NO_CHANNELS_MSG)
    if len(channels) > 1:
       ...
       if not toid:
          return await message.reply_text(Translation.WRONG_CHANNEL_MSG, reply_markup=ReplyKeyboardRemove())
    else:
    ...
    if fromid.text and not fromid.forward_date:
        ...
        if not match:
            return await message.reply(Translation.INVALID_LINK_MSG)
        ...
        if last_msg_id == None:
           return await message.reply_text(Translation.FORWARDED_FROM_GROUP_MSG)
    else:
        await message.reply_text(Translation.INVALID_MSG)
        return 
    try:
  ...
  #  except ChannelInvalid:
        ...
        return await message.reply(Translation.INVALID_LINK_SPECIFIED_MSG)
...
async def contact_admin_callback(client, callback_query):
    """Handle contact admin button"""
    from config import Config
    
    await callback_query.answer(
        f"📞 Contact admin at: {Config.ADMIN_CONTACT_URL}",
        show_alert=True
    )
