...
async def start_command(client, message):
    ...
    if len(message.command) > 1:
        ...
        try:
            result = await db.set_user_referred_by(user_id, referral_code)
            if result:
                logger.info(f"Successfully set referral for user {user_id} with code {referral_code}")
            else:
                logger.warning(f"Failed to set referral for user {user_id} with code {referral_code}")
        except Exception as e:
            logger.error(f"Error setting referral for user {user_id}: {e}")
    ...
    if user_data and user_data.get('referred_by') and not user_data.get('referral_completed'):
        logger.info(f"User {user_id} was referred and is already subscribed, completing referral")
        ...
        if referral_result and isinstance(referral_result, dict) and referral_result.get('completed'):
            try:
                from plugins.fsub import _send_referral_completion_notifications
...
#==================Speedtest Command==================#
# Speedtest command removed - now handled by PTB in ptb_all_commands.py
...
# Speedtest callback removed - now handled by PTB only
    ...
    try:
        ...
        await query.message.edit_text(
            text="<b>🎉 Free Trial Activated!</b>\n\n"
                 ...
                 "Upgrade to Premium:\n"
                 f"• <b>Plus Plan:</b> ₹{Config.PLAN_PRICING['plus']['15_days']}/15d, ₹{Config.PLAN_PRICING['plus']['30_days']}/30d - Unlimited forwarding\n"
                 f"• <b>Pro Plan:</b> ₹{Config.PLAN_PRICING['pro']['15_days']}/15d, ₹{Config.PLAN_PRICING['pro']['30_days']}/30d - Unlimited + FTM Mode + Priority support\n\n"
...
async def admin_user_stats_callback(client, callback_query):
    ...
    try:
        ...
        # Revenue calculation
        estimated_revenue = (pro_users * 299) + (plus_users * 199)
