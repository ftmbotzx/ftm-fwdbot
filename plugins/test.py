...
async def get_configs(user_id):
    ...
    try:
        ...
        # Migrate ftm_prefix and ftm_suffix from filters to root config if they exist there
        if 'ftm_prefix' not in user_data or user_data['ftm_prefix'] is None:
            if 'filters' in user_data and 'ftm_prefix' in user_data['filters']:
                user_data['ftm_prefix'] = user_data['filters']['ftm_prefix']
                # Remove from filters after migration
                del user_data['filters']['ftm_prefix']
            else:
                user_data['ftm_prefix'] = None
        
        if 'ftm_suffix' not in user_data or user_data['ftm_suffix'] is None:
            if 'filters' in user_data and 'ftm_suffix' in user_data['filters']:
                user_data['ftm_suffix'] = user_data['filters']['ftm_suffix']
                # Remove from filters after migration
                del user_data['filters']['ftm_suffix']
            else:
                user_data['ftm_suffix'] = None
...
async def update_configs(user_id, key, value):
    ...
    try:
        ...
        if key in ['caption', 'duplicate', 'db_uri', 'forward_tag', 'protect', 'file_size', 'size_limit', 'extension', 'keywords', 'button', 'ftm_mode', 'ftm_prefix', 'ftm_suffix']:
            current[key] = value
        else:
